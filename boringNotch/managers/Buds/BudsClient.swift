    ...
    public func setLiveUpdatesEnabled(_ enabled: Bool) {
        ...
        if liveUpdatesEnabled != enabled {
            if enabled {
                ...
                if debugEnabled {
                    print("[LIVE] off after \(dt)s (\(liveRxPackets) pkts, \(liveRxBytes) bytes, \(pps) pkt/s, \(bps) B/s)")
                }
                liveRxStartedAt = nil
            }
        }
...
extension BudsClientImpl: @preconcurrency CBCentralManagerDelegate, @preconcurrency CBPeripheralDelegate {
    ...
    public func centralManager(_ central: CBCentralManager,
        ...
        guard let name, isTargetBudsName(name) else { return }
    ...
    public func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if bytes.count >= 12, bytes[safe: 4] == 0x06, bytes[safe: 5] == 0x81, bytes[safe: 6] == 0x25 {
            if let b = OPOParse.parseBattery(bytes) {
                battery = b
                batterySeq &+= 1
                if debugEnabled { print("Battery parsed L=\(b.left ?? -1) R=\(b.right ?? -1) Case=\(b.case ?? -1)") }
            }
            return
        }
        ...
        if let b = OPOParse.parseBatteryTelemetry(bytes) {
            ...
            battery = b
            batterySeq &+= 1
            if debugEnabled { print("Battery telemetry L=\(b.left ?? -1) R=\(b.right ?? -1) Case=\(b.case ?? -1)") }
        }
        ...
        if bytes.count >= 7, bytes[safe: 4] == 0x05 {
            if let e = OPOParse.parseEQ(bytes) {
                ...
                if !merged.presets.isEmpty {
                    if debugEnabled { print("EQ parsed presets=\(merged.presets.count) current=\(merged.currentPreset ?? -1)") }
                }
            }
            return
        }
